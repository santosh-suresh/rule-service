...
function findOcurrencesBetween (rules = [], startDate, endDate, targetTz = 'Etc/UTC') {
  const parsedRules = rules.map((rule) => {
    const option = RRule.parseString(rule.rrule)
    const startTimeInTz = new Date(`${rule.start_time}Z`)
    const endTimeInTz = new Date(`${rule.end_time}Z`)
    const duration = (endTimeInTz - startTimeInTz)
    const startTime = moment.utc(rule.start_time).format('HH:mm:ssZ')
    option.dtstart = startTimeInTz
